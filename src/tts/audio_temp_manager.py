...
class AudioTempManager:
    ...
    def save_audio(
        self, audio_data: np.ndarray, sample_rate: int, prefix: str = "audio"
    ) -> str:
    ...
    def convert_audio_format(
        self, source_path: str, target_path: str, target_format: AudioFormat
    ) -> str:
        if target_format not in SUPPORTED_FORMATS:
            raise ValueError(
                f"不支持的音频格式: {target_format}，支持: {SUPPORTED_FORMATS}"
            )
        ...
        try:
            ...
            raise RuntimeError(
                "pydub 未安装，无法进行格式转换。请安装: pip install pydub"
            )
        except Exception as e:
        ...
        try:
            ...
            if actual_format == "wav":
                ...
                return self.convert_audio_format(
                    temp_wav, full_path_no_ext, actual_format
                )
        finally:
    ...
    def get_audio_to_format_target_path(
        self, save_dir: str, prefix: str = "audio", output_format: AudioFormat = "wav"
    ) -> str:
        from datetime import datetime
    ...
    def save_to_persistent(
        ...
        if actual_format == "wav":
            ...
            return self.convert_audio_format(
                temp_file_path, full_path_no_ext, actual_format
            )
